/**
 * @fileoverview HLJSL language registry (codes, display names) and fast heuristic language detection.
 */
...
/**
 * Fast heuristic language detection (sampling, weighted patterns, early termination).
 */
export class LanguageDetector {
    ...
    #checkSpecialCases(code, scores) {
        ...
        if (maxScore < 30) {
            ...
            for (let i = 0; i < maxLinesToCheck; i += 1) {
                if (/^\s{4,}/.test(lines[i])) {
                    indentedLines += 1;
                }
            }
